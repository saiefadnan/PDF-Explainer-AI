...
from functions import chunk_text_with_tiktoken, query_pinecone_for_context
from functions import download_nltk_resources, get_expanded_answer, get_summary_for_text
from functions import generate_summary_pdf, get_summaries_for_chunks, validate_api_keys
...
max_tokens_per_chunk = st.sidebar.number_input(
    "Max Tokens per Chunk", 
    min_value=200, 
    max_value=2000, 
    value=500, 
    step=50,
    help="Larger = fewer API calls but less granular"
)
...
st.sidebar.markdown("---")
st.sidebar.markdown("Developed with ❤️ using Streamlit & AI")
...
if uploaded_file is not None:
    ...
    if "current_file_name" not in st.session_state or st.session_state.current_file_name != uploaded_file.name:
        ...
        st.session_state.preprocessed_text = ""
        st.session_state.chat_history = []
        st.session_state.pdf_id = None  # Reset namespace for new file
    ...
    if not st.session_state.processing_done:
        ...
        st.markdown(f"""
            <div style='background-color: #f8f9fa; border-left: 4px solid #0084ff; padding: 12px; margin-bottom: 12px; border-radius: 4px;'>
                ...
                <div style='font-size: 14px; color: #1a1a1a; line-height: 1.6;'>
                    {st.session_state.preprocessed_text[:1000]}{'...' if len(st.session_state.preprocessed_text) > 1000 else ''}
                </div>
            </div>
        ...
        if st.session_state.preprocessed_text:
            ...
            with st.spinner('Chunking text with tiktoken...'):
                text_chunks = chunk_text_with_tiktoken(
            ...
            if text_chunks:
                ...
                if st.session_state.chunk_summaries:
                    with st.spinner("Preparing PDF..."):
                        try:
                            ...
                            st.session_state.pdf_bytes = summary_pdf_bytes
                        except Exception as e:
    ...
    # Show cached results if already processed
        ...
        if st.session_state.extracted_text:
            st.write(f"Original text length: {len(st.session_state.extracted_text)} characters.")
        ...
        if st.session_state.text_chunks:
            st.write(f"Number of text chunks created: {len(st.session_state.text_chunks)}")
    ...
    if user_input and user_input.strip():
        with st.spinner('🤔 Thinking...'):
            try:
                ...
                relevant_context = query_pinecone_for_context(user_input, top_k=5)
                ...
                st.rerun()  # NEEDED: Refresh to display new message
                
            except Exception as e:
                error_msg = f"Sorry, I encountered an error: {str(e)}"
                st.session_state.chat_history.append((user_input, error_msg))
                st.rerun()  # NEEDED: Refresh to display error message
    
    # Clear chat button
    if st.session_state.chat_history:
        if st.button("🗑️ Clear Chat History"):
            st.session_state.chat_history = []
            st.rerun()  # NEEDED: Refresh to remove chat display
    ...
    if not st.session_state.chat_history:
        ...
        with col1:
            if st.button("📋 What is this document about?"):
                ...
                st.rerun()  # NEEDED: Refresh to process pending question
